...
export const generateHtmlCode = (texts: CalculatorTexts, loanParams: LoanParams): string => {
  ...
  return `<div id="loan-calculator"><div class="loan-calc-header"><h1>${texts.title}</h1><p>${texts.subtitle}</p></div><div class="loan-calc-card"><div class="loan-calc-group"><div class="loan-calc-label"><span>${texts.amountLabel}</span><span class="loan-calc-value" id="amount-value">${defaultAmount.toLocaleString('ru-RU')} ₽</span></div><input type="range" min="${loanParams.minAmount}" max="${loanParams.maxAmount}" step="${loanParams.stepAmount}" value="${defaultAmount}" class="loan-calc-slider" id="amount-slider"><div class="loan-calc-hint">${texts.amountHint}</div></div><div class="loan-calc-group"><div class="loan-calc-label"><span>${texts.daysLabel}</span><span class="loan-calc-value" id="days-value">${defaultDays} дней</span></div><input type="range" min="${loanParams.minDays}" max="${loanParams.maxDays}" step="${loanParams.stepDays}" value="${defaultDays}" class="loan-calc-slider" id="days-slider"><div class="loan-calc-hint">${texts.daysHint}</div></div>${texts.button1Link ? `<a href="${texts.button1Link}" target="_blank" rel="noopener noreferrer" class="loan-calc-btn">${texts.button1Text}</a>` : `<button class="loan-calc-btn">${texts.button1Text}</button>`}${texts.button2Link ? `<a href="${texts.button2Link}" target="_blank" rel="noopener noreferrer" class="loan-calc-btn loan-calc-btn-primary">${texts.button2Text} <span id="total-amount">${defaultAmount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span> ₽</a>` : `<button class="loan-calc-btn loan-calc-btn-primary">${texts.button2Text} <span id="total-amount">${defaultAmount.toLocaleString('ru-RU', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span> ₽</button>`}</div></div>`;
};
